...
class T1_Standing_One_Leg(BaseTask):
    ...
    def _init_buffers(self):
        ...
        self.left_feet_contact_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.float)
    ...
    def _reset_idx(self, env_ids):
        ...
        self.left_feet_contact_buf[env_ids] = 0.0
    ...
    def step(self, actions):
        ...
        left_feet_contact = self.feet_contact[:, 0].float()
        # Update contact buffer using tensor operations
        self.left_feet_contact_buf = torch.where(
            left_feet_contact > 0,
            self.left_feet_contact_buf + 1.0,
            torch.zeros_like(self.left_feet_contact_buf)
        )
    ...
    def _check_termination(self):
        ...
        self.reset_buf |= self.left_feet_contact_buf > np.ceil(self.cfg["rewards"]["left_feet_on_ground_length_s"] / self.dt)
    ...
    def _reward_left_feet_z(self):
        # Get the height of the left foot (z-coordinate)
        left_foot_height = self.feet_pos[:, 0, 2]
        
        # Get the reference height from config
        target_height = self.cfg["rewards"]["feet_distance_ref_z"]
        
        # Calculate the difference between actual and target height
        height_diff = -torch.abs(left_foot_height - target_height)
        
        # Create a reward that decreases as the difference increases
        reward = torch.exp(2.0 * (height_diff - 1.0) + 2)
        
        print(f"Z-axis: left_foot_height={left_foot_height.mean().item():.4f}, target={target_height:.4f}, height_diff={height_diff.mean().item():.4f}, reward={reward.mean().item():.4f}")
        
        return reward
    
    def _reward_left_feet_touching_ground(self):
        # Check if the left foot is touching the ground
        left_foot_touching_ground = self.feet_contact[:, 0].float()
        
        # Create a reward that increases as the left foot is touching the ground
        reward = left_foot_touching_ground
        return reward
