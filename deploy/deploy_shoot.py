...
from utils.policy_shoot import Policy
...
class Controller:
    def __init__(self, cfg_file) -> None:
        ...
        self.running = True
        self.start_time = None  # Track when main loop starts
    ...
    def run(self):
        ...
        # Check if 2 seconds have passed and stop running
        current_time = time.perf_counter()
        if current_time - self.start_time >= 2.0:
            self.running = False
            self.logger.info("2 seconds elapsed, stopping controller")
            return
